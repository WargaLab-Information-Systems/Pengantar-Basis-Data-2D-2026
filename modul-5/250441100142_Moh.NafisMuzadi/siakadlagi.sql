...
SELECT MIN(sks) AS SKS_Minimal, 
MAX(sks) AS SKS_Maksimal, 
AVG(sks) AS Rata_Rata_SKS, 
COUNT(kode_mk) AS Total_Mata_Kulia 
FROM mata_kuliah;
...
SELECT m.nim, m.nama, sum(mk.sks) AS total_sks, n.nilai_angka
FROM mahasiswa m
JOIN krs k
ON m.nim = k.nim
JOIN mata_kuliah mk
ON k.kode_mk = mk.kode_mk
join nilai n
on n.nim = m.nim and n.kode_mk = mk.kode_mk
where k.semester in (1,3)
and n.nilai_angka > 80
GROUP BY m.nim, m.nama, n.nilai_angka;